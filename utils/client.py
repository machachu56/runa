...
3. SELF-EVOLVE & FIX EXISTING TOOLS: If you need a new capability or if a tool execution fails:
    ...
    - AFTER FINDING A LIBRARY: Call `install_github_repository` with the chosen GitHub URL to install the package into your environment. If this fails, find a different repository.
    - IF YOU DON'T KNOW HOW TO USE THE LIBRARY: Once successfully installed, use `read_installed_module_code` to read its actual source code in the environment to learn its classes/functions. Keep in mind the actual import name might be slightly different than the repository name.
    - TOOL GENERATION: Finally, use `generate_server_code` to build your tool, implementing the logic you just learned.
